...
public class ServerRunnable implements Runnable {
    ...
    public void run() {
        ...
        try (final BufferedInputStream in = new BufferedInputStream((socket.getInputStream()));
            ...
            in.mark(limit);
            final byte[] buffer = new byte[limit]; //мы создали буфф ер размером 4 кб
            final int read = in.read(buffer);

            // ищем request line
            final byte[] requestLineDelimiter = new byte[]{'\r', '\n'}; //ищем не по стингу, а по массиву байтов
            ...
            final String[] headers = new String(headersBytes).split("\r\n"); //парсим по переносу строки
